...
-- GAP-024 Gate A activation/history coupling follow-up.
--
-- The append-only projection-history table records every pair that reaches canonical
-- classification. Prevent a current authority pair from being superseded or retired
-- until that exact (value_revision, evidence_basis_revision) pair is present in history.
-- This closes the skipped-intermediate-pair gap without changing the public readers or
-- inventing another business revision identity.
...
comment on function public.hr_guard_attendance_fact_activation() is
  'GAP-024 Gate A activation guard: forward-only authority changes require the previously current value/evidence pair to have durable canonical classification history.';
