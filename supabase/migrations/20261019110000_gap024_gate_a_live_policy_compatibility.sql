-- GAP-024 Gate A live authorization-substrate compatibility correction.
--
-- The original Gate-A migration has already been applied to the live Supabase project.
-- The confirmed current authorization schema uses:
--   * public.entity_policies(entity_id, policy_id) for direct/global assignments;
--   * public.policies(id, key, ...) for capability keys;
--   * public.house_roles + public.roles + public.role_policies for requested-House role grants;
--   * public.platform_roles + public.roles + public.role_policies for platform role grants.
--
-- Preserve the approved authorization semantics without replaying the historical
-- flattened entity_policies view migrations:
--   * exact requested-House membership is mandatory;
--   * owner/manager aliases stay out of the branch-limited reader;
--   * direct/PLATFORM capability may satisfy feature-read capability only;
--   * branch scope comes only from requested-House role policy assignments;
--   * every parsed branch must belong to the requested House.
--
-- Public RPC signature and return shape are unchanged.
...
begin
  ...
  with actor as (
    ...
    -- (entity_id, policy_id). Direct grants are global capability only; they never
    -- create requested-House membership or branch scope.
...
comment on function public.hr_read_canonical_attendance_branch_scoped(
  uuid, date, date, uuid, integer, integer
) is 'GAP-024 Gate A sanitized branch-scoped reader using the confirmed live direct/platform capability and requested-House role-policy authorization substrate.';
