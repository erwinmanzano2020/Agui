-- GAP-024 Gate A replay guard for the live-policy compatibility reader.
--
-- 20261019110000 is already applied to the restored live Supabase project. The live
-- entity_policies object is a direct (entity_id, policy_id) table, but historical
-- repository replay can expose entity_policies as the older flattened effective-policy
-- view with scope/scope_ref columns. This forward-only correction preserves identical
-- live behavior while preventing HOUSE/GUILD rows from that historical view from being
-- treated as global feature capability.
--
-- Public signature, return shape, House-membership requirement, House branch restriction,
-- owner/manager exclusion, grants and PostgREST reload remain unchanged.
...
comment on function public.hr_read_canonical_attendance_branch_scoped(
  uuid, date, date, uuid, integer, integer
) is 'GAP-024 Gate A sanitized branch-scoped reader using live direct/platform capability, historical PLATFORM-only flattened compatibility, and requested-House role-policy branch scope.';
