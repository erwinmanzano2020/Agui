...
-- GAP-024 Gate A retired-fact authority-pointer freeze.
--
-- A retired fact is a tombstone of its final classified authority pair. Projection
-- rebuilds intentionally exclude inactive facts, so neither a later update while retired
-- nor the retirement UPDATE itself may advance value/evidence authority pointers.
-- Preserve the prior activation/history and lineage semantics while rejecting both shapes.
...
comment on function public.hr_guard_attendance_fact_activation() is
  'GAP-024 Gate A SECURITY DEFINER activation guard: retired facts freeze their final classified authority pair, and active authority changes require durable prior classification history.';
