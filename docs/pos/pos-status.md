...
| Implemented, Not Closed | POS-F3 Slice 11 — Payment Method Selection Runtime conforms to Slice 11C as amended by Slice 11D. It enforces the exact two-member trusted-record input, observable unknown-member and accessor rejection, five-value vocabulary, and exact `PAYMENT_METHOD_SELECTED` result without downstream effects. Slice 11 is not closed or locked, and Slice 9 authority remains unchanged. See [`pos-f3-slice-11-runtime-implementation.md`](../devlog/pos-f3-slice-11-runtime-implementation.md). |
...
14. Treat Slice 11C as Implementation Approval Only and preserve the implemented deterministic, read-only Payment Method Selection runtime exactly within its frozen contract. Slice 11 Runtime is Implemented, Not Closed; do not treat implementation as closure, locking, tender handling, payment execution, or canonical downstream payment-processing authority.
...
16. Treat Slice 7 checkout container event vocabulary, state-event consistency rules, and checkout container boundary model as governance-only boundary language (conceptual events + integrity anchors + invalidation terminology), not runtime authorization.
17. Preserve Roadmap gates: inventory-coupled POS work requires Operations authority and an approved inventory integration contract; settlement/accounting work requires Finance authority and an approved accounting/settlement integration contract.
18. Maintain conservative no-leak/scope-first/operator-attributed posture as non-negotiable continuation rules.
