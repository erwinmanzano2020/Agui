-- Rewrite HR lookup RPC to avoid ambiguous references and qualify columns
...
do $$
begin
  if not exists (select 1 from pg_proc where proname = 'mask_identifier_value') then
    execute $$
...
begin
  ...
  with matches as (
    ...
    where v_email is not null
      and (
        (ei.identifier_type = 'EMAIL' and ei.identifier_value = v_email)
        or (ei.kind = 'email' and ei.value_norm = v_email)
      )
    union all
    ...
    where v_phone_e164 is not null
      and (
        (ei.identifier_type = 'PHONE' and ei.identifier_value = v_phone_e164)
        or (ei.kind = 'phone' and ei.value_norm = coalesce(v_phone_legacy, v_phone_e164))
      )
    union all
    ...
    where v_phone_legacy is not null
      ...
      and ei.kind = 'phone'
      and ei.value_norm = v_phone_legacy
  ),
  aggregated as (
    select
      m.entity_id,
