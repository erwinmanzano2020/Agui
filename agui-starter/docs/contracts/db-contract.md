...
- **Identity link:** `employees.entity_id` is nullable and links to canonical `entities.id`. When email/phone is provided at creation, resolve or create an `entity` + `entity_identifiers` entry; leave `entity_id` null when no contact is provided. Keep the column nullable for legacy rows.
