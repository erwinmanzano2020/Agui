...
- **Employee creation:** Inserts must set `house_id`, use `full_name` for names (no `display_name` column), default `status` to `active` unless explicitly provided, and ensure `branch_id` belongs to the same house before insert.
