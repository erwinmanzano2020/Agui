...
- Lookup-first flow: call `hr_lookup_entities_by_identifiers` (masked results, HR-scoped) to surface candidates and require explicit selection. If RPCs are missing, reload PostgREST schema after applying migrations.
...
- Lookup-first enrollment: treat phone/email as **weak** identifiers (non-unique, recyclable). Do not auto-merge when ambiguous—surface candidates and require explicit selection. Reserve “strong” identifiers (future: verified gov IDs, issued QR/loyalty tokens) for higher-confidence matches. Medium-strength: controlled loyalty cards with issuer metadata.
- Emergency contacts are **not** identity identifiers; store separately (do not reuse for identity linking).
- Identifier lifecycle (future): add/manage multiple identifiers, mark primary, soft-disable instead of deleting; always normalize and check duplicates before adding.
- Schema cache: any migration that adds/changes RPCs must end with `notify pgrst, 'reload schema';` to avoid stale PostgREST metadata and “function not found in schema cache” errors.
