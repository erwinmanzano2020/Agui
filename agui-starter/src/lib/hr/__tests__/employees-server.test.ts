...
type QueryResult = { data: EmployeeRowWithHouse[] | null; error: { message: string } | null };

class EmployeesQueryMock {
  constructor(private rows: EmployeeRowWithHouse[], private result: QueryResult = { data: null, error: null }) {}
  ...
  eq(_column: string, value: string) {
    const filtered = this.rows.filter((row) => row.house_id === value);
...
const baseRow: EmployeeRowWithHouse = {
  id: "emp-1",
  entity_id: "ent-1",
  brand_id: "brand-1",
  code: "E-01",
...
describe("listEmployeesForHouse", () => {
  it("returns only employees for the requested house sorted by name", async () => {
    const supabase = new SupabaseMock([
      baseRow,
      { ...baseRow, id: "emp-2", code: "E-02", full_name: "Grace Hopper", house_id: "house-1" },
      { ...baseRow, id: "emp-3", code: "E-03", full_name: "Alan Turing", house_id: "house-2" },
    ]);
