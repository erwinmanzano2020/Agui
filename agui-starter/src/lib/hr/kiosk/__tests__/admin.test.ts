      ...
      return {
        ...
        maybeSingle() {
          ...
          if (table === "hr_kiosk_devices") {
            ...
            return Promise.resolve({
              data: state.deviceExists && state.deviceHouseId === "house-1"
...
describe("kiosk admin", () => {
  ...
  it("denies cross-branch mutation for branch-limited actors", async () => {
    ...
    mock.method(access, "requireHrAccess", async () => ({
      ...
      policyKeys: [
        "tiles.hr.read",
        "tiles.hr.branch.00000000-0000-0000-0000-000000000001",
      ],
      entityId: "entity-2",
    }));
  ...
  it("returns 404 for authorized actor when device does not exist in house", async () => {
    const { supabase, state } = createSupabaseStub();
    state.deviceExists = false;
    await assert.rejects(
      () => rotateKioskDeviceToken(supabase as never, { houseId: "house-1", deviceId: "missing-device" }),
      (error: unknown) => error instanceof KioskAdminError && error.status === 404,
    );
  });

  it("allows manager to rotate token across branches", async () => {
    const { supabase, state } = createSupabaseStub();
    state.deviceBranchId = "branch-2";
    const result = await rotateKioskDeviceToken(supabase as never, { houseId: "house-1", deviceId: "device-1" });
    assert.ok(result.plaintextToken.length > 10);
  });
});
