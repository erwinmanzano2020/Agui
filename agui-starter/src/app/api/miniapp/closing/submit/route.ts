...
import type { MiniAppClosingSubmitPayload, MiniAppClosingSubmitResponse } from "@/lib/miniapp/types";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

function json(body: MiniAppClosingSubmitResponse, status = 200) {
...
export async function POST(request: Request) {
  let body: { initData?: string; payload?: MiniAppClosingSubmitPayload };
  try {
    body = (await request.json()) as { initData?: string; payload?: MiniAppClosingSubmitPayload };
  } catch {
  ...
  if (!payload) return json({ ok: false, code: "PAYLOAD_MISSING", message: "Closing payload is missing." }, 400);
  ...
  let result: MiniAppClosingSubmitResponse;
  try {
    result = (await upstream.json()) as MiniAppClosingSubmitResponse;
  } catch {
    return json({ ok: false, code: "UPSTREAM_BAD_RESPONSE", message: "Agui closing service returned an invalid response." }, 502);
  }

  if (!result.ok) {
    const status = result.code === "UNAUTHORIZED_PROXY" ? 502 : result.code?.startsWith("INIT_") ? 401 : 409;
    return json(result, status);
  }
  if (result.mode !== "CONTROLLED_SUBMIT_PILOT" || result.action !== "CASHIER_CLOSE_SUBMIT" || result.status !== "CLOSED") {
    return json({ ok: false, code: "UPSTREAM_MODE_MISMATCH", message: "Agui closing service did not return the approved controlled-submit result." }, 502);
  }
  return json(result);
}
