...
export async function POST(request: Request) {
  ...
  if (
    ...
    return NextResponse.json(
      { ok: false, error: "Missing key/scope/value" },
      { status: 400 },
    );
  }

  const { key, scope, value, businessId, branchId } = payload as Record<string, unknown>;
  ...
  if (scope === "GM" && role.role !== "GM") {
    return unauthorizedResponse("Only GM may edit GM defaults");
  }
    ...
    await setSetting(
      {
        key: typedKey,
        scope: typedScope,
        value: value as SettingValueForKey<typeof typedKey>,
